<?php
// controllers/LandingSlideController.php
// Manages Landing page slide CRUD (Create, Read, Update, Delete, Toggle Status)

class LandingSlideController {
    private $landingSlideModel;

    public function __construct() {
        $this->landingSlideModel = new LandingSlide();
    }

    /**
     * Creates a new landing slide
     */
    public function create() {
        ...
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            ...
            // Handle file upload for landing image
            $imageUrl = null;
            if (isset($_FILES['slide_image']) && $_FILES['slide_image']['error'] === UPLOAD_ERR_OK) {
                ...
                if ($imageUrl === false) {
                    ...
                    header('Location: ' . BASE_URL . 'admin?tab=landing');
                    exit();
                }
            } else {
            ...
            $success = $this->landingSlideModel->create([
                'title' => $title,
                'image_url' => $imageUrl,
            ...
            if ($success) {
                $_SESSION['success'] = 'เพิ่มรูปภาพสไลด์หน้าก่อนเข้าสำเร็จเรียบร้อยแล้ว';
            } else {
    ...
    public function edit() {
        ...
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            ...
            $slide = $this->landingSlideModel->getById($id);
            if (!$slide) {
            ...
            $success = $this->landingSlideModel->update($id, $data);
            if ($success) {
                $_SESSION['success'] = 'แก้ไขรูปภาพสไลด์หน้าก่อนเข้าสำเร็จเรียบร้อยแล้ว';
            } else {
    ...
    public function delete() {
        ...
        if ($slide) {
            ...
            $success = $this->landingSlideModel->delete($id);
            if ($success) {
                $_SESSION['success'] = 'ลบรูปภาพสไลด์หน้าก่อนเข้าสำเร็จเรียบร้อยแล้ว';
            } else {
    ...
    public function toggleStatus() {
        ...
        if ($slide) {
            ...
            $success = $this->landingSlideModel->update($id, [
                'title' => $slide['title'],
    ...
    /**
     * Core Helper: Handles sanitisation and upload of landing images
    ...
    private function handleImageUpload($file) {
        ...
        $newFilename = 'landing_' . time() . '_' . bin2hex(random_bytes(4)) . '.' . $extension;
    ...
    /**
     * Reorders landing slides via AJAX
     */
    public function reorder() {
        ...
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            ...
            if (is_array($order) && !empty($order)) {
                ...
                try {
                    $db->beginTransaction();
                    $stmt = $db->prepare("UPDATE landing_slides SET display_order = :display_order WHERE id = :id");
